...
**现象：** 点击「安卓系统」卡片 → 跳转到 `05-预置APK` 而非 `00-学习计划`。

**原因：** 首页导航卡片使用 `.RegularPages.ByDate` 取第一篇文章，但日期是按文件创建时分配的，与编号顺序不一致。文件名 `00-` 的文章可能日期比 `05-` 晚（不同批次的时间分配不同）。

**解决：** 改为 `.RegularPages.ByTitle`，依赖文件编号自然排序（00→01→02...）。有子分类的卡片，取第一个子分类的第一篇。

---

## 18. 跳转后侧边栏不定位当前文章

**现象：** 从首页点卡片跳转到文章页后，侧边栏滚动位置停留在之前的状态，不显示当前激活的文章。

**原因：** 侧边栏只在页面加载时恢复 `sessionStorage` 中记录的旧滚动位置，没有主动滚到 `a.active` 所在位置。

**解决：** 页面加载时优先执行 `active.scrollIntoView({ block: 'center' })`，找不到激活项才回退到记忆位置。
